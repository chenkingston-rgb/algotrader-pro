"""
seed_rulebook.py — One-shot script to create the StrategyRulebook entity schema
in the AlgoTrader Pro Base44 app and seed all 31 strategy rules.

Run via GitHub Actions (uses BASE44_SERVICE_TOKEN secret).
"""
...
# ─── Schema creation ─────────────────────────────────────────────────────────
# Base44 entity schema is created implicitly on first POST if not present,
# but we define it explicitly via the schema endpoint.
SCHEMA = {
    "type": "object",
    "properties": {
        "category": {
            "type": "string",
            "enum": [
                "Global Risk", "Market Regime", "Symbol Routing",
                "Strategy: bollinger_bands_15m", "Strategy: momentum_roc_15m",
                "Strategy: macd_crossover", "Strategy: triple_ema",
                "Strategy: ema_crossover", "Strategy: rsi_macd_combo",
                "Position Sizing", "VIX Filters", "Exit Logic", "Watchlist / Scanner"
            ]
        },
        ...
        "status":         {"type": "string", "enum": ["Active", "Deprecated", "Under Review"]},
    }
}
...
RULES = [
    {"category":"Global Risk","rule_name":"Risk per trade","status":"Active","version_added":"v5",
     ...
     "implementation":"RISK_PCT = 0.01 → dollar_risk = equity × 0.01 × vix_mult",
    ...
    {"category":"Global Risk","rule_name":"Max position cap","status":"Active","version_added":"v5",
     ...
     "implementation":"MAX_POSITION_PCT = 0.10 → max_by_cap = (equity × 0.10) / price. Final qty = min(shares_by_risk, max_by_cap)",
    ...
    {"category":"Global Risk","rule_name":"ATR stop-loss multiplier","status":"Active","version_added":"v5",
     "description":"Stop-loss placed at 1.5× ATR(14) below entry on every buy.",
     "implementation":"ATR_STOP_MULT = 1.5 → stop_price = entry - (1.5 × ATR14)",
    ...
    {"category":"Global Risk","rule_name":"Default take-profit multiplier","status":"Active","version_added":"v5",
     "description":"Default take-profit at 3.0× ATR(14) above entry. Overridden dynamically by the VWAP TP multiplier if VWAP is available.",
     "implementation":"ATR_TP_MULT = 3.0 → tp_price = entry + (3.0 × ATR14). Overridden by get_tp_multiplier() if VWAP available.",
    ...
    {"category":"Global Risk","rule_name":"ATR floor (minimum stop distance)","status":"Active","version_added":"v6 / 2026-05-26",
     ...
     "implementation":"min_atr = price × 0.0020; eff_atr = max(atr, min_atr). Applied in both main loop and _run_streaming_strategy.",
    ...
    {"category":"Global Risk","rule_name":"Kill switch — max drawdown","status":"Active","version_added":"v5",
     ...
     "implementation":"MAX_DRAWDOWN_PCT = 25.0. Checked each run: drawdown_pct = (equity - peak_equity) / peak_equity × 100",
    ...
    {"category":"Market Regime","rule_name":"SPY 20-day MA regime filter","status":"Active","version_added":"2026-05-28",
     ...
     "implementation":"spy_df = get_bars(\'SPY\', \'1Day\', 30); spy_ma20 = rolling(20).mean(); regime_ok = spy_price > spy_ma20. Sets os.environ[\'REGIME_OK\'] = \'1\' or \'0\'.",
    ...
    {"category":"Market Regime","rule_name":"ADX regime size scaler (SPY)","status":"Active","version_added":"v5",
     "description":"Position size scaled by SPY ADX(14). Trending (ADX≥23 + rising) = 1.0×. Neutral = 0.75×. Sideways (ADX<18) = 0.50×. Stacks with VIX and RSI(2) multipliers.",
     "implementation":"get_regime_size_multiplier(): ADX≥23 + slope>0 → TRENDING → 1.0×; ADX<18 → SIDEWAYS → 0.5×; else → NEUTRAL → 0.75×. Cached every 30 min.",
    ...
    {"category":"Symbol Routing","rule_name":"Pool-aware watchlist routing","status":"Active","version_added":"2026-05-28",
     ...
     "rationale":"Without routing, both strategies received the same list. Momentum entered ranging stocks; Bollinger tried to mean-revert trending ones. The scanner already computed the right classification."},
    ...
    {"category":"Watchlist / Scanner","rule_name":"Weekly scanner — TREND pool criteria","status":"Active","version_added":"v2 scanner",
     ...
     "implementation":"score_trend(): hard filters (price $5-$2000, avg_vol_30d > 300K, within 30% of 52w high, ret_12w > -15%) + ADX14 > 20. Score = 0.40×ret_4w + 0.40×ret_12w + 0.20×ADX.",
    ...
    {"category":"Watchlist / Scanner","rule_name":"Weekly scanner — MEAN_REV pool criteria","status":"Active","version_added":"v2 scanner",
     ...
     "implementation":"score_mean_rev(): hard filters + ADX 10-22 + ret_4w between -8% and +8%. Score = 0.60×(100/realized_vol_5d) + 0.40×(1 - |price/SMA20 - 1|×10).",
    ...
    {"category":"Watchlist / Scanner","rule_name":"Daily scanner — intraday gap scoring","status":"Active","version_added":"v2 scanner",
     ...
     "implementation":"score_daily(): score = |gap_pct|×0.6 + (pm_vol/avg_daily_vol×100)×0.4. Minimum: |gap_pct| ≥ 0.10 OR pm_vol_ratio ≥ 0.005. TREND threshold: score ≥ 3.0.",
     "rationale":"Pre-market gap + elevated volume is the strongest intraday signal of institutional-driven momentum."},
    ...
    {"category":"VIX Filters","rule_name":"VIX block — MEAN_REV strategies","status":"Active","version_added":"v5",
     ...
     "rationale":"Mean reversion assumes price returns to a stable mean. At VIX ≥ 22, bands expand dramatically and mean-reversion assumption breaks down."},
    ...
    {"category":"Position Sizing","rule_name":"RSI(2) position size multiplier","status":"Active","version_added":"v5",
     "description":"Position size scaled by SPY RSI(2). RSI2 < 10 = 1.3× (oversold). RSI2 > 90 = 0.7× (overbought). Mid-range = 1.0×.",
     "implementation":"get_rsi2_size_multiplier(): RSI2<10→1.3×; RSI2<30→1.1×; RSI2>90→0.7×; RSI2>70→0.85×; else→1.0×. Stacks with VIX and regime multipliers.",
     "rationale":"RSI(2) on SPY is a fast short-term mean reversion indicator. Extremely oversold = size up entries. Extremely overbought = reduce exposure."},
    {"category":"Position Sizing","rule_name":"VWAP take-profit multiplier","status":"Active","version_added":"v5",
     "description":"Take-profit dynamically scaled by distance from intraday VWAP. Far above VWAP = 3.0× ATR. Near VWAP = 2.5×. Below VWAP = 2.0×.",
     "implementation":"get_tp_multiplier(price, vwap): price/vwap > 1.005 → 3.0×; 0.995-1.005 → 2.5×; < 0.995 → 2.0×. Returns 3.0 if VWAP unavailable.",
     "rationale":"Entries well above VWAP have more room to run — extend the target. Entries near or below VWAP face resistance sooner — take profits earlier."},
    {"category":"Strategy: bollinger_bands_15m","rule_name":"Entry condition","status":"Active","version_added":"v6 / 2026-05-26",
     "description":"Buy when: price < lower Bollinger Band (20-period, 2σ) AND price is above the 50-bar MA. Both conditions required simultaneously.",
    ...
    {"category":"Strategy: bollinger_bands_15m","rule_name":"Exit condition","status":"Active","version_added":"v6 / 2026-05-26",
     ...
     "implementation":"price > bb_upper → sell. bb_upper = MA20 + 2.0 × std(20).",
    ...
    {"category":"Strategy: momentum_roc_15m","rule_name":"Core ROC signal — sustained momentum","status":"Active","version_added":"v6 / 2026-05-26",
     ...
     "implementation":"roc = (close / close.shift(10) - 1) × 100. Buy: r > 0.3 AND r > prev_r. Sell: r < -0.3 AND r < prev_r.",
    ...
    {"category":"Strategy: momentum_roc_15m","rule_name":"Volume confirmation filter","status":"Active","version_added":"2026-05-27",
     "description":"Buy signals require current bar volume ≥ 1.0× the 20-bar average volume. Exits (sells) are never volume-gated.",
     "implementation":"vol_ok = cur_vol ≥ 1.0 × avg_vol(20). Buy blocked if not vol_ok → skip_reason=\'vol_filter_long\'. Params: vol_ma=20, vol_threshold=1.0.",
    ...
    {"category":"Strategy: macd_crossover","rule_name":"Signal logic","status":"Active","version_added":"v5",
     "description":"Buy when MACD histogram crosses from negative → positive. Sell when crosses from positive → negative. Timeframe: 1Day.",
    ...
    {"category":"Strategy: triple_ema","rule_name":"Signal logic","status":"Active","version_added":"v5",
     ...
     "rationale":"Three-EMA alignment is high-conviction trend confirmation. \'First bar only\' avoids re-entering an already-running trend mid-move."},
    ...
    {"category":"Exit Logic","rule_name":"Position exit when dropped from watchlist","status":"Active","version_added":"v5",
     ...
     "implementation":"dropped = [s for s in position_symbols if s not in pools[\'all\']]. Prepended to all strategy symbol lists.",
...
BASE_URL = f"https://superagent-f5f9b72f.base44.app/api/apps/{BASE44_APP_ID}/entities/StrategyRulebook"

# First create the entity schema
schema_url = f"https://superagent-f5f9b72f.base44.app/api/apps/{BASE44_APP_ID}/entities/StrategyRulebook/schema"
sr = requests.post(schema_url, headers=hdrs, json={"schema": SCHEMA}, timeout=10)
print(f"Schema create: {sr.status_code} — {sr.text[:200]}")

# Check if entity already exists
check = requests.get(f"https://superagent-f5f9b72f.base44.app/api/apps/{BASE44_APP_ID}/entities/StrategyRulebook/", headers=hdrs, timeout=10)
...
for rule in RULES:
    r = requests.post(
        f"https://superagent-f5f9b72f.base44.app/api/apps/{BASE44_APP_ID}/entities/StrategyRulebook/",
        headers=hdrs, json=rule, timeout=10
    )
    if r.ok:
        created += 1
        print(f"  ✓ {rule[\'rule_name\'][:50]}")
    else:
        failed += 1
        print(f"  ✗ {rule[\'rule_name\']} — {r.status_code} {r.text[:100]}")
    time.sleep(0.1)
