...
| S18 | Dead-man, alerts and an independent scheduling path | GitHub schedule, Cloudflare cron enable/dispatch, heartbeat, native D1 alert journal, optional Discord/Slack alerts, status schema | startup-red-status and alert tests; native delivery plus restricted-token workflow-read/direct-paper-dispatch checks passed; first automatic Worker-originated dispatch gate open |
